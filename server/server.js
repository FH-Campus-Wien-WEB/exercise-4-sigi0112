...
app.use(session({
  ...
  cookie: { secure: false, httpOnly: false, path:'/' } // Set to true if using HTTPS
}));
...
app.get("/movies", requireLogin, function (req, res) {
  ...
  if (queriedGenre) {
    movies = movies.filter((movie) => movie.Genres && movie.Genres.indexOf(queriedGenre) >= 0);
  }
    ...
    fetch(url, { signal: controller.signal })
      ...
      .then(movieData => {
        if (movieData && movieData.Response === 'True') {
          ...
          // Saubere Aufbereitung aller Felder für den MovieBuilder deines Lehrers
          const internalMovie = {
  imdbID: movieData.imdbID,
  Title: movieData.Title || "Unknown Title",
  Released: movieData.Released && movieData.Released !== "N/A" ? movieData.Released : new Date().toISOString(),
  Runtime: movieData.Runtime ? parseInt(movieData.Runtime, 10) || 0 : 0,
  Genres: movieData.Genre && movieData.Genre !== "N/A" ? movieData.Genre.split(", ").map(g => g.trim()) : [],
  Directors: movieData.Director && movieData.Director !== "N/A" ? movieData.Director.split(", ").map(d => d.trim()) : [],
  Writers: movieData.Writer && movieData.Writer !== "N/A" ? movieData.Writer.split(", ").map(w => w.trim()) : [],
  Actors: movieData.Actor && movieData.Actor !== "N/A" ? movieData.Actor.split(", ").map(a => a.trim()) : [],
  Plot: movieData.Plot && movieData.Plot !== "N/A" ? movieData.Plot : "",
  Poster: movieData.Poster && movieData.Poster !== "N/A" ? movieData.Poster : "",
  Metascore: movieData.Metascore && movieData.Metascore !== "N/A" ? parseInt(movieData.Metascore, 10) : null,
  imdbRating: movieData.imdbRating && movieData.imdbRating !== "N/A" ? parseFloat(movieData.imdbRating) : null
};
      ...
      .catch((err) => {
        ...
        if (err.name === 'AbortError') {
          console.error('OMDb API details request timeout');
          return res.sendStatus(504);
        }
        console.error('OMDb API details error:', err);
        res.sendStatus(500);
      });
